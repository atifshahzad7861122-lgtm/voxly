...
def download_video(youtube_url: str) -> Path:
    ...
    cmd = [
        ...
        "--no-playlist",
        "--no-part",
        # Use android client — supports cookies, no PO token, no SABR, no JS runtime needed
    ...
    with _cookie_lock:
        if COOKIES_FILE.exists() and COOKIES_FILE.stat().st_size > 200:
            ...
            print("[*] Using cookies.txt for download", flush=True)
...
def _pollinations_generate(prompt: str, output_path: Path,
    ...
    try:
        ...
        print(f"[Thumb/AI] Pollinations error: {e}", flush=True)
        return False
...
def cut_clip(video_path: Path, start: float, end: float,
    ...
    r1 = subprocess.run(base_cmd, capture_output=True)
    if r1.returncode != 0:
...
def upload_cookies():
    """Accept a cookies.txt upload from the UI."""
    data = request.get_json(force=True, silent=True) or {}
